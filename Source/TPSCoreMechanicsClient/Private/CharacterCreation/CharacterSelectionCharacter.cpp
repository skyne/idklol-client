...
{
	// Convert uint8 to native enum
	ECharacterSkinColor SkinColor = static_cast<ECharacterSkinColor>(CharacterCreator.SkinColor);
...
void ACharacterSelectionCharacter::UpdateParameters(FCharacterCreatorTemplate& CharacterCreator)
{
	...
	if (Parameters.Gender != CharacterCreator.Gender)
	{
		...
		// Convert uint8 to native enum and use helper
		ECharacterGender Gender = static_cast<ECharacterGender>(CharacterCreator.Gender);
